...
used. In C\#, you can now declare a variable \code{var} and let the
...
\src{snippet10}
In C\# you would write this method as:
...
\end{snip}
but a C\# \code{enum} is secretly an integer, and its values must be
qualified with the type name, as in \code{Bool.True} and
\code{Bool.False}.
...
\code{isAlpha} or \code{isDigit}. In C\# the \code{System.Char} type
provides similar predicates like \code{Char.IsLetter} and
\code{Char.IsDigit}, and these return a proper \code{bool} rather than
an integer.
