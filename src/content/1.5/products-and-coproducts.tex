...
\src{snippet08}
In C\#, we would use generic methods, for instance:
...
types: it's a tagged union of two types. C\# doesn't provide untagged
unions, so you typically combine a tag --- an enumeration --- with the
alternative fields, and keep track of which one is valid. For instance, a
tagged union of an \code{int} and a \code{string} could be implemented
as:

\begin{snip}{csharp}
struct Contact {
    public enum Kind { IsPhone, IsEmail }
    public Kind Tag;
    public int PhoneNum;
...
A tagged union is also called a \newterm{variant}. C\# does not have a
built-in variant type, but third-party libraries such as \code{OneOf}
provide a general implementation.
