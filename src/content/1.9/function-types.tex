...
nontrivial. You can think of multi-argument methods in C\# as
corresponding to Haskell functions taking tuples (although, to confuse
things even more, in C\# you can define methods that take an explicit
tuple, as well as \code{params} variadic methods, and methods taking
collection expressions).

You can partially apply a C\# function by capturing arguments in a
lambda. For instance, given a function of two strings:
...
\begin{snip}{csharp}
Func<string, string> greet = s => catstr("Hello ", s);
Console.Write(greet("Haskell Curry"));
\end{snip}
...
For another example, let's look at the C\# type \code{byte}, which
contains 256 values (Haskell \code{Char} is larger, because Haskell
uses Unicode). Many character-classification predicates, restricted to
the 256 code points that fit in a byte, are usually implemented using
lookups. Functions like \code{Char.IsUpper} or \code{Char.IsWhiteSpace}
correspond to tables, which are equivalent to tuples of 256 Boolean
values. A
...
``multiply'' \code{bool} by itself 256 (or \code{byte}) times, you
get \code{bool} to the power of \code{byte}, or \code{bool}\textsuperscript{\code{byte}}.

How many values are there in the type defined as 256-tuples of
\code{bool}? Exactly $2^{256}$. This is also the number
of different functions from \code{byte} to \code{bool}, each
function corresponding to a unique 256-tuple. You can similarly
calculate that the number of functions from \code{bool} to
\code{byte} is $256^{2}$, and so on. The exponential
