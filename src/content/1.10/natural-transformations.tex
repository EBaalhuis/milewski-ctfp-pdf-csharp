...
\code{Select}, \code{ContinueWith}, and the like in C\#).
...
functions and C\# generic methods, and it's reflected in the way these
...
C\#, on the other hand, also supports \newterm{ad hoc polymorphism}, in
the form of method overloading: the same method name can be given
entirely different definitions for different argument types, and the
compiler picks the right one based on the types at the call site. Its
generics, however, are genuinely parametric --- a generic method is
type-checked once against its constraints, so it must be well-defined for
every type that satisfies them.

In C\#, method overloading similarly allows different definitions of the
same operation for different types. In Haskell this functionality is
provided by type classes and type families.
