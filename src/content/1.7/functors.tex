...
the same result. C\# doesn't have text-substitution macros that would
perform this kind of inlining behind your back, but the same caution
applies whenever a ``function'' hides side effects.
...
Let's consider the C\# analog of \code{Maybe}, a generic type
\code{Optional}. Here's a sketch of the implementation:

\begin{snip}{csharp}
class Optional<T> {
    private readonly bool _isValid; // the tag
    private readonly T _v;
    public Optional() { _isValid = false; }            // Nothing
    public Optional(T x) { _isValid = true; _v = x; }  // Just
    public bool IsValid() => _isValid;
    public T Val() => _v;
}
\end{snip}
This generic type provides one part of the definition of a functor: the
mapping of types. It maps any type \code{T} to a new type
\code{Optional<T>}. Let's define its action on
functions:

\begin{snip}{csharp}
Func<Optional<A>, Optional<B>> fmap<A, B>(Func<A, B> f) {
    return opt => {
        if (!opt.IsValid())
            return new Optional<B>();
        else
            return new Optional<B>(f(opt.Val()));
    };
}
\end{snip}
...
\begin{snip}{csharp}
Optional<B> fmap<A, B>(Func<A, B> f, Optional<A> opt) {
    if (!opt.IsValid())
        return new Optional<B>();
    else
        return new Optional<B>(f(opt.Val()));
...
\end{snip}
There is also an option of making \code{fmap} an instance method of
...
inherit from (unfortunately, you can't have a generic virtual method
parameterized by the container type)? Should it be a curried or an
uncurried free generic method? Can the
...
generic class, like \code{Optional}, so by analogy, we would like to
parameterize \code{fmap} with a \newterm{higher-kinded type parameter}
\code{F} --- something like this:

\begin{snip}{csharp}
F<B> fmap<F, A, B>(Func<A, B> f, F<A> x);
\end{snip}
Unfortunately, C\# has no higher-kinded types: a type parameter
\code{F} cannot be used as a type constructor and applied to another
type, as in \code{F<A>}. So there is no way to write a single generic
\code{fmap} that works for every functor. You can't even name the
functor as a type argument:

\begin{snip}{csharp}
Optional<B> fmap<Optional, A, B>(Func<A, B> f, Optional<A> opt);
\end{snip}
Instead, we have to fall back on method overloading, which brings us
...
\code{List<T>}, which could be considered the most generic C\#
container. The implementation of \code{fmap} for \code{List<T>}
is just a thin encapsulation of LINQ's \code{Select}:

\begin{snip}{csharp}
List<B> fmap<A, B>(Func<A, B> f, List<A> v) {
...
\code{IEnumerable<T>}, which can be transformed with LINQ's
\code{Select} --- the more primitive cousin of \code{fmap}. LINQ
makes the functorial nature of sequences quite pronounced.
...
\code{Task<T>}, which may at some point contain a value, but it's
...
\begin{snip}{csharp}
Const<C, B> fmap<C, A, B>(Func<A, B> f, Const<C, A> c) {
    return new Const<C, B>(c._v);
}
\end{snip}
