...
in C\# it's provided by the built-in value tuple types.
...
values, like empty strings, negative numbers, null references, etc. This
...
\src{snippet29}
More complex sum types are often faked in C\# using references. A
reference can be either null, or refer to a value of a specific type. For instance,
a Haskell list type, which can be defined as a (recursive) sum type:

\src{snippet30}
can be translated to C\# using the null reference trick to implement the
empty list:

\begin{snip}{csharp}
class List<A> {
    Node<A> _head;
    public List() { _head = null; }     // Nil
    public List(A a, List<A> l)         // Cons
    {
...
the special \code{null} value for \code{\_head} to encode
\code{Nil}.
...
understood as one variant type, in which the runtime type serves as a
...
dispatching a call to a virtual method based on the object's runtime
type.

C\# has no built-in \code{union} type at all, so this class-hierarchy
approach (or a library variant type) is the usual way to encode a sum.
