...
application in imperative languages, including C\#. I'm talking
about resumable functions or coroutines. It's not a secret that
...
looks very much like sequential code. C\#'s \code{async}/\code{await}
makes the same transformation possible. And the same mechanism can be applied to
turn the
\urlref{https://bartoszmilewski.com/2014/04/21/getting-lazy-with-c/}{spaghetti
  of nested loops} into list comprehensions or ``generators'' (in C\#,
\code{yield return}), which are
