...
string toUpper(string s) {
    return new string(s.Select(char.ToUpper).ToArray());
}
\end{snip}
...
in a generic way by defining a generic type \code{Writer} that
encapsulates a pair whose first component is a value of arbitrary type
\code{A} and the second component is a string:

\begin{snip}{csharp}
record struct Writer<A>(A Value, string Log);
\end{snip}
Here are the embellished functions:

\begin{snip}{csharp}
Writer<string> toUpper(string s) {
    var result = new string(s.Select(char.ToUpper).ToArray());
    return new Writer<string>(result, "toUpper ");
}

Writer<List<string>> toWords(string s) {
    return new Writer<List<string>>(words(s), "toWords ");
}
\end{snip}
...
Writer<List<string>> process(string s) {
    var p1 = toUpper(s);
    var p2 = toWords(p1.Value);
    return new Writer<List<string>>(p2.Value, p1.Log + p2.Log);
}
\end{snip}
...
Writer<bool> isEven(int n) {
    return new Writer<bool>(n % 2 == 0, "isEven ");
}
\end{snip}
...
Writer<bool> negate(bool b) {
    return new Writer<bool>(!b, "Not so! ");
}
\end{snip}
...
Writer<bool> isOdd(int n) {
    var p1 = isEven(n);
    var p2 = negate(p1.Value);
    return new Writer<bool>(p2.Value, p1.Log + p2.Log);
}
\end{snip}
...
C\#, we have to use a generic method parameterized by three types
...
\begin{snip}{csharp}
Func<A, Writer<C>> compose<A, B, C>(Func<A, Writer<B>> m1,
                                    Func<B, Writer<C>> m2)
{
    return x => {
        var p1 = m1(x);
        var p2 = m2(p1.Value);
        return new Writer<C>(p2.Value, p1.Log + p2.Log);
    };
}
\end{snip}
Now we can go back to our earlier example and implement the composition
of \code{toUpper} and \code{toWords} using this new method:

\begin{snip}{csharp}
Writer<List<string>> process(string s) {
    return compose<string, string, List<string>>(
        toUpper, toWords)(s);
}
\end{snip}
There is still a lot of noise with the passing of types to the
\code{compose} method. We can write the composition more concisely by
using an expression-bodied method and deconstructing each \code{Writer}
pair directly:
...
Func<A, Writer<C>> compose<A, B, C>(Func<A, Writer<B>> m1,
    ...
    x => {
        var (b, log1) = m1(x);
        var (c, log2) = m2(b);
        return new Writer<C>(c, log1 + log2);
    };
\end{snip}
Thanks to C\#'s type inference, we no longer need to spell out the type
arguments, so the implementation of \code{process} simplifies to:
...
\src{snippet01}
Here I'm just defining a type alias, an equivalent of a \code{using}
alias in C\#. The type \code{Writer} is parameterized by
...
\src{snippet06}
The function \code{map} corresponds to the C\# \code{Select}. It
...
class Optional<A> {
    ...
    public Optional(A v) { _isValid = true; _value = v; }
...
Optional<double> safe_root(double x) {
    if (x >= 0) return new Optional<double>(Math.Sqrt(x));
    else return new Optional<double>();
}
\end{snip}
